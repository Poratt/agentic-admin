...
## 2026-09-18 — Model metadata enrichment (OpenRouter catalog) + Edit-Model dialog redesign

- **Decision — enrichment is inline at `syncProviderModels`, not a queue/cron.** Sync is the 95% entry path for new models, so the catalog fetch happens once there (in-memory map lookup per key, zero per-model network). The catalog service still caches the whole payload for 24h (+ negative cache), so the Edit-Model button and a second sync are free. Tradeoff accepted: a model added directly (not via sync) stays empty until someone hits Auto-Detect — the button exists exactly for that.
- **Decision — unknown is NULL, free is 0, and the service never throws.** A missing or failed catalog fetch must never block a model write. Prices are stored per-1M-tokens (matches every display surface); `free_tier` is derived from the `:free` suffix or both prices being 0.
- **Decision — bare-name index built from each catalog entry's own `id` only, never `alias_target.slug`.** The live catalog's 18 `~`-prefixed "auto-latest" rows point their alias at the real entry; indexing both inflates ambiguity 2-3× and would have forced NULLs on keys that match uniquely. `~` is always a prefix (verified 18/18, zero infix), so `split('~')[0]` was rejected outright (it returns an empty string on prefix-`~`).
- **Decision — seeds go through the same enrichment before insert.** Bootstrap idempotent: a dev/new-DB boot lands filled, an existing table is untouched (the seed already early-returns when non-empty).
- **Decision — the detect endpoint is `HIDDEN_FROM_LLM`.** It is admin diagnostic plumbing; exposing it would burn an agent turn on a tool that duplicates the sync path.
- **Decision — free-tier prices are `disable()`d from the component, not `[disabled]` in the template.** Template `disabled` next to `formControlName` produces an Angular console warning; the reactive path is `disable()/enable()` + `getRawValue()` at save, which still carries the locked 0s to the backend.
- **⚠️ Lesson (Golden Rule candidate) — `(ngModelChange)` on a `formControlName`d control whose handler writes back to that control recurses silently.** PrimeNG's `writeValue` re-fires the output → infinite recursion → `RangeError: Maximum call stack size exceeded`. It did NOT fail any test: vitest reports uncaught exceptions as warnings and the suite looked green. The tell was 25 unhandled errors in the tail of the report. Fix: use `(onChange)` (emitted from the native input's `change` handler, never from `writeValue`). Detecting this class of bug requires reading the unhandled-error count, not just the pass count.
- **⚠️ Lesson re-learned — Golden Rule #8 bites on EVERY dialog change, not just the first one.** A dialog redesign can pass every unit test and every build and still render completely unstyled: the CSS was written in the component file, `p-dialog` projects to `<body>`, and emulated `[_ngcontent]` scoping silently never matched the projected nodes. The tell this time: the classes that DID work in the same dialog (`.form-field`, `.dialog-form`, `.toggle-group`) were all global, while the broken ones were component-local. Procedural fix adopted: **after any dialog markup change, verify the compiled `dist/.../styles-*.css` contains the new selectors WITHOUT `[_ngcontent]`** — that is the only proof the styles reach the projected DOM.
- **Decision — the dialog redesign is a token-only CSS card, not the user's inline-style sketch.** The sketch (inline styles, raw hex colors) was structural guidance; the implementation uses `color-mix` + `var(--token)` so dark/light and RTL stay correct. The Free Tier checkbox moved into the pricing header per the sketch and the two price inputs dim+disable when it is on.

## 2026-09-18 — static tool tier filtering: keyword domain groups (Phase 1 of a staged plan)

- **Decision — deterministic keyword/regex selection FIRST; Tool-RAG (embeddings) only after measurement.** The feature is the first rung: in-process regex picks domain groups from the USER prompt, merged with a fixed always-on set. A low-confidence prompt returns the full 75-tool set, so capability loss is impossible by construction. Phase 2 (Tool-RAG over `summaryHe` + tags, top-K=8) and Phase 3 (10-prompt benchmark) are explicitly deferred until this ships and is measured — no speculative embedding infra.
- **Decision — filter lives in a separate `ToolTierFilterService`, not as a `SwaggerToolsParser` method.** The parser owns spec-loading/hiding; the filter is a pure, prompt-in → subset-out transform, unit-testable in isolation without a parser instance. The domain data is a plain constants file (`constants/tool-domain-groups.ts`) so new tags/keywords are a constants edit + redeploy — no DB, no dynamic config (YAGNI for 16 tags).
- **Decision — MCP tools are never filtered.** They join in `AdminAgentService.getTools()` AFTER the tier filter and keep their existing always-on grouping untouched — zero risk to a separately-owned integration surface (GitHub/Notion-grade external behavior).
- **Decision — `HIDDEN_FROM_LLM` runs BEFORE the tier filter (in `parser.getTools()`), which is safe here:** the fallback path returns exactly `getTools()` output, so it can never expose the 6 hidden ops. (The plan's "hide after the keyword pass" caution is satisfied by the existing split by construction — the filter only ever sees already-hidden output.)
- **Decision — Hebrew keyword regexes are bare substrings, not `\b`-boundary regexes.** JS `\b` matches ASCII word-characters only (`\w = [a-zA-Z0-9_]`); Hebrew letters are not word chars, so `/\bזן\b/` would never fire on Hebrew text. Bare Hebrew substrings occasionally over-match (safe direction: keeps MORE tools), while every Latin keyword keeps `\b`.
- **Decision — untagged tools are always kept.** A tool with `tags.length === 0` cannot be classified; dropping it would violate the "never fewer tools than before" guarantee, so the filter keeps any untagged tool on the filtered path.
- **Changed (8 files):** `swagger-tools.parser.ts` (`LlmToolSchema.tags?: string[]`, capture `op.tags ?? []`; +4 spec), NEW `constants/tool-domain-groups.ts`, NEW `services/tool-tier-filter.service.ts` (+7 spec), `admin-agent.service.ts` (`getTools(prompt?)`, both call sites pass the user prompt; MCP joins unfiltered after), `admin-agent.module.ts` (service registered), `admin-agent.service.spec.ts` (4 constructor sites).
- **Verification:** targeted 42/42; full backend `--runInBand` **573 passed / 4 failed = 3 pre-existing suites only** (baseline 562 → +11 new, zero new failures); `npm run build -w backend` exit 0.

- **Decision — Option A (stats parity): `is_test` on the existing `llm_call_stats` table, not a new stats table.** `llm_model_test_results` remains an audit-only JSON store; `getModelStats` now reads `AVG`/`MIN` latency, `AVG(toolCallReliability)` and `MAX(createdAt)` from `llm_call_stats` for BOTH ping and real halves, keyed on `is_test`. The old `WHERE caller <> 'health'` filter is deleted; `recordCallStat` no longer short-circuits on `caller === 'health'`. Tradeoff accepted: two halves of the stats source share a single table and are distinguished only by a boolean column.
- **Decision — video → `skipped`, not a real ping.** Agnes bills on the first `200 OK` and no safe dry-run / cancel mechanism has been verified. The ping records a `'skipped'` row in `llm_model_test_results` (provider never called) and returns `{ success: true, available: false }` to the frontend, so the user sees the "Skipped" badge instead of a false failure.
- **Decision — default capability throws, does not silently skip.** An unknown `capability` value on a model row persists an `'error'` row in the audit log, THEN returns `400 No test implemented for capability: X`. This makes the missing-path visible in the test history instead of a silent no-op.
- **Decision — server-side `capability` column on `llm_model_test_results`.** The frontend already knows the capability from the model row at the time of the test call; storing it server-side lets the statistics view group / display historical pings by capability without reconstructing state, and decouples the test row from a possibly-stale frontend classification.
- **Decision — `HEALTH_CALLER` const deleted, caller column becomes provenance-only.** Now that the health check no longer writes to `llm_call_stats`, the `caller` field on `LlmCallStatEntity` no longer has a distinguishing value — the `is_test` boolean is the sole discriminator; the column comment was updated to reflect this.
- **Decision — jest cap `maxWorkers: 2` AND prefer `--runInBand` for agent runs.** The RAM incident (ts-jest per-file compilation × all cores + orphaned killed workers) was resolved by capping `maxWorkers: 2` in `backend/package.json`; the agent rule additionally requires `--runInBand` so a single Node process avoids even two ts-jest compile-in-memory workers. The config cap is a hard backstop; the flag is an agent-level best practice.
- **Decision — repo-level `CLAUDE.md` and `AGENTS.md` diverge.** The new rule was added to `AGENTS.md` and `~/.claude/CLAUDE.md`; the repo `CLAUDE.md` (duplicate of AGENTS.md) was NOT updated in this session to avoid an unrelated diff in a feature branch. Synchronize on the next commit to `main`.
